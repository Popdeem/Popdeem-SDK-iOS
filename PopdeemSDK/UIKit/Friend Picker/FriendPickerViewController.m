...
- (void)viewDidLoad {
    ...
    [_tableView setFrame:CGRectMake(_tableView.frame.origin.x, _tableView.frame.origin.y, self.view.frame.size.width, _tableView.frame.size.height)];
...
-(UITableViewCell *)imageCell :(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (friend.selected) {
        [imageView setImage:[UIImage imageNamed:@"friend_selected"]];
    } else {
        [imageView setImage:[UIImage imageNamed:@"friend_deselect"]];
    }
